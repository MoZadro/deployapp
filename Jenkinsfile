...
pipeline {
    ...
    environment {
        ...
        NAMESPACE = "xxx-xxxxxxxxxx" // change
        TEAM_NAME = "xxx"  //svn, int, cas, ngs

        DOCKER_REGISTRY = "docker-app.nsoft.com:1088x" // 10882-seven 10886-casino 10887-integrations 
        DOCKER_REGISTRY_CREDENTIALS_ID = "the-jenkins-credentials-id" //change (provided via LP ... jenkins.bot.xxx and neeeds to be stored in jenkins credentials)
        DOCKER_IMAGE = "some-image-name" //change (project name or something)

        TRANSCRYPT_CREDENTIALS_ID = "transcrypt-project-name" //change
